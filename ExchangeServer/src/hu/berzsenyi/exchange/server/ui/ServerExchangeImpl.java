...
public class ServerExchangeImpl extends JFrame implements
		ServerExchange.IServerExchangeListener, WindowListener,
		ComponentListener {
	private static final long serialVersionUID = 301475774323683701L;

	public static void main(String[] args) {
		new ServerExchangeImpl(false);
	}
	...
	protected JButton testBtnNewEvent;

	public ServerExchangeImpl(boolean test) {
		super("Exchange");
		try {
			...
			ServerExchange.INSTANCE.open(Integer.parseInt(JOptionPane
					.showInputDialog(this, "Port", "8080")), JOptionPane
					.showOptionDialog(this, "Which gamemode?",
							"Choose gamemode", JOptionPane.DEFAULT_OPTION,
							JOptionPane.QUESTION_MESSAGE, null, options,
							options[0]), Double.parseDouble(JOptionPane
					.showInputDialog(this, "Startmoney", "10000")));
			ServerExchange.INSTANCE.load(JOptionPane.showInputDialog(this,
					"Load backup from", "backup/save0.dat"));
			repaint();
		} catch (Exception e) {
			e.printStackTrace();
			System.exit(1);
	...
	private void onResize() {
		...
		SwingHelper.splitComponentsVertical(new Rectangle(barX, barY + by / 3,
				barWidth / 6, barHeight - by / 3), 0.25, btnNewEvent,
				btnStockUpdate);
		SwingHelper.setButtonTextSizeToFill(btnNewEvent);
		SwingHelper.setButtonTextSizeToFill(btnStockUpdate);
		if (btnNewEvent.getFont().getSize() < btnStockUpdate.getFont()
				.getSize())
			btnStockUpdate.setFont(btnNewEvent.getFont());
		else
			btnNewEvent.setFont(btnStockUpdate.getFont());

		SwingHelper.splitComponentsVertical(new Rectangle(barX + barWidth / 6
				+ bx / 2, barY + by / 3, barWidth / 8, barHeight - by / 3),
				0.25, radioTabStocks, radioTabPlayers, radioTabEvents);
	}
