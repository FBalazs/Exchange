...
public class EventQueue extends Event {
	...
	public static class Builder {
		...
		private ServerExchange mExchange;

		public Builder(ServerExchange exchange) {
			mExchange = exchange;
			mMultipliers = new double[mExchange.getStockNumber()];
		...
		public Builder setMultipliers(String multipliersAsString) {
			...
			for (String split : splits) {
				...
				int stockIndex = 0;
				for (; stockIndex < mExchange.getStockNumber(); stockIndex++)
					if (mExchange.getStock(stockIndex).id.equals(stockName))
						break;
				if (stockIndex < mExchange.getStockNumber())
