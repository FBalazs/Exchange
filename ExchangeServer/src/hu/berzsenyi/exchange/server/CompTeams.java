...
public class CompTeams extends DisplayComp {
	...
	public void paint(Graphics g) {
		...
		for(int t = 0; t < this.model.teams.size(); t++) {
			...
			int h1 = this.model.round == 0 ? (int)(this.getHeight()/1.1) : (int) (this.getHeight()*this.model.teams.get(t).getMoney()/maxMoney);
			int h2 = this.model.round == 0 ? 0 : (int) (this.getHeight()*this.model.teams.get(t).getStockValue(this.model)/maxMoney);
			...
			drawStringCentered(this.model.teams.get(t).name, x+w/2, this.getHeight()-h1-h2-g.getFontMetrics().getHeight(), g);
			drawStringCentered(""+(this.model.round == 0 ? this.model.startMoney : this.model.teams.get(t).getMoney()+this.model.teams.get(t).getStockValue(this.model)), x+w/2, this.getHeight()-h1-h2, g);
		}
